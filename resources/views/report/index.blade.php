...
@section('page_subtitle', 'Pantau omzet dan transaksi')

@section('content')
<div class="space-y-4" x-data="{ type: 'harian', date: '{{ now()->format('Y-m-d') }}', month: {{ now()->month }}, year: {{ now()->year }} }">
    ...
    <div class="bg-white rounded-lg shadow p-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            ...
            <div x-show="type === 'harian'">
                ...
                <input type="date" x-model="date" class="input-field" :max="'{{ now()->format('Y-m-d') }}'">
            </div>
            ...
            <div class="flex gap-2">
                <button @click="
                    if(type==='harian') window.location.href='{{ route('report.harian') }}?date='+date;
                    else window.location.href='{{ route('report.bulanan') }}?month='+month+'&year='+year;
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <a href="{{ route('report.harian', ['date' => now()->format('Y-m-d')]) }}"
        ...
        <a href="{{ route('report.bulanan', ['month' => now()->month, 'year' => now()->year]) }}"
            class="bg-white rounded-lg shadow p-5 hover:shadow-md transition group">
            <div class="flex items-center gap-4">
